...
function lookupOption(option) {
	const idx = cCmdLineOptionsDef.findIndex((def) => (def.name === option));
	return (idx >= 0) ? cCmdLineOptionsDef[idx] : undefined;
}
...
function printResult(e) {
	...
	console.log("\n" + (options.dryRun ? "[DRY RUN] " : "") + "Tailoring npm module for environment '" +
		options.environment.name + "' " + (!e ? "SUCCEEDED" : "FAILED"));
}
...
try {
	...
	if (cStates.indexOf(persistedTailoringState) < 0) {
		...
		if (persistedTailoringState === options.environment.name) {
			...
			console.log("ERROR: The npm module has been already tailored for environment '" +
				persistedTailoringState +
				". \nPlease uninstall and re-install @dynatrace/oneagent module to tailor for environment '" +
