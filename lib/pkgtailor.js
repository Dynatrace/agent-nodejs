...
const cStateLambdaWithNode4 = makeAwsTailorStateName(4);
const cStateLambdaWithNode6 = makeAwsTailorStateName(6);
const cStateLambdaWithNode8 = makeAwsTailorStateName(8);
const cStates = [
	cStateNotTailored,
	cStateUndefined,
	cStateLambdaWithNode4,
	cStateLambdaWithNode6,
	cStateLambdaWithNode8
];
...
const cCmdLineOptionsDef = [
	...
	makeAwsLambdaCmdLineDef(4),
	makeAwsLambdaCmdLineDef(6),
	...
	{
		name: "--AwsLambdaV8",
		verbose: "Tailor environment for AWS Lambda with Node.js V8.x",
		handler: () => {
			debug("setting environment to AWS cLambdaV8EnvDef");
			options.environment = cLambdaV8EnvDef;
		}
	}
	*/
];
...
function makeAwsLambdaCmdLineDef(nodeVer) {
	...
	let environment = {
		...
		deleteFilePredicate: (fn) => {
			return deletePattern.test(fn) || c32BitPathPattern.test(fn);
		}
	};
...
function makeDeleteFilePattern(nodeVer) {
	...
	if (apiVer.length !== 2) {
		throw new Error("expected 2 digit api version, got '" + apiVer + "'");
	}
	
	const x = apiVer.charAt(0);
	const y = apiVer.charAt(1);
	const patternStr = `_(${x}[^${y}]|[^${x}][0-9])\\.node$`;
